...
#include "yolo_ws/utils.h"
#include "yolo_ws/yolov8Predictor.h"

class YoloObjectTrackingNode : public rclcpp::Node {
public:
    YoloObjectTrackingNode()
        : Node("yolo_object_tracking_node"),
          modelPath_("model/model.onnx"), // Path to your YOLO model
          classNamesPath_("model/coco.names"), // Path to your class names file
          confThreshold_(0.5f), iouThreshold_(0.5f), maskThreshold_(0.6f), isGPU_(true),
          initialized_(false) {

        RCLCPP_INFO(this->get_logger(), "YoloObjectTrackingNode started");
